...
class PaymentSystem:
    ...
    def effective_network_params(self) -> Tuple[float, float, float]:
        ...
        if self.cfg.enable_degradation:
            ...
            loss = min(self.cfg.degrade_max_loss, loss + self.cfg.degrade_loss_alpha * overload)
    ...
    for cfg in SCENARIOS:
        for tps in LOAD_POINTS_TPS:
            for run_idx in range(NUM_RUNS):
                seed = BASE_SEED + (zlib.crc32(cfg.key.encode("utf-8")) % 100000) * 1000 + int(tps) * 10 + run_idx
